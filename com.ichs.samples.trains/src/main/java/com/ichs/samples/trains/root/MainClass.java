...
public class MainClass {
    public static void main(final String[] args) {
        try {
            ...
            final MapAggregate newMap = CountryMapFactory.getInstance().createMap(args[0]);
            final IRoute route = newMap.getShortestRoutesStartingEnding(args[1], args[2]);
            System.out.println(String.format("Shortest path between %s and %s is %s", args[1], args[2], route));
        } catch (UnAcceptableInputParameterException | LoopingRoadException | RepeatedRoadException
                | TownNotExistException | NoRouteExistsException e) {
            System.out.println(e.getMessage());
            System.out.println("Usage");
            System.out.println("-------------");
            System.out.println("call with three arguments");
            System.out.println("Argument 1: the map description E.g. AB5, BC4, CD8, DC8, DE6, AD5, CE2, EB3, AE7");
