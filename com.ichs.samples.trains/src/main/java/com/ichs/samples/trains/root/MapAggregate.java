...
/**
 * Class that holds the map of the country. It contains the utility methods for manipulating the roads and towns. It is
 * the single Aggregate root that can be accessed by clients.
 *
 */
@NonNullByDefault
public class MapAggregate {
    ...
    public ITown getTown(final String townName) throws UnAcceptableInputParameterException, TownNotExistException {
        ...
        if (CountryMapFactory.allowedCityNames.contains(townName)) {
            ...
            throw new UnAcceptableInputParameterException(
                    String.format("Letter %s is not an acceptable city", townName), ErrorCodeEnum.e1006);
        }
        return originTown;
    }
    ...
    public IRoad getRoad(final String startTownName, final String endTownName)
            throws UnAcceptableInputParameterException, TownNotExistException, RoadNotExistException {
        ...
        if (index < 0) {
            throw new RoadNotExistException(String.format("Road %s does not exist in the map.", tempRoad),
                    ErrorCodeEnum.e1010);
        }
    ...
    /**
     * returns all the Routes starting from startTown to finishTown with exact number of stops
     *
     * @param startTownName
    ...
    public List<IRoute> getRoutesStartingEndingExact(final String startTownName, final String finishTownName,
            final int exactStop) throws UnAcceptableInputParameterException, TownNotExistException,
        ...
        final List<String> paths = getPath(exactStop - 1, startTown, finishTown, startTownName);
    ...
    /**
     * returns all the Routes starting from startTown to finishTown with maximum number of stops or less
     *
     * @param startTown
     *            name of the start Town, must be in the map
     * @param finishTown
     ...
     * @throws UnAcceptableInputParameterException
     */
    public List<IRoute> getRoutesStartingEndingMaxStop(final String startTownName, final String finishTownName,
            final int maxStop) throws UnAcceptableInputParameterException, TownNotExistException,
            NoRouteExistsException {
        assert maxStop > 0;
        final ArrayList<IRoute> returnVal = new ArrayList<IRoute>();
        for (int i = 1; i < maxStop + 1; i++) {
            try {
                returnVal.addAll(getRoutesStartingEndingExact(startTownName, finishTownName, i));
    ...
    public IRoute getShortestRoutesStartingEnding(final String startTown, final String finishTown)
            throws UnAcceptableInputParameterException, TownNotExistException, NoRouteExistsException {
        final List<IRoute> allRoutes = getRoutesStartingEndingMaxStop(startTown, finishTown, this.getNumberOfTowns());
        Collections.sort(allRoutes);
        return allRoutes.get(0);
    }

    /**
     * returns all the Routes starting from startTown to finishTown with distance shorter than the specified value
     *
     * @param startTown
     *            name of the start Town, must be in the map
     * @param finishTown
     ...
     */
    @SuppressWarnings("null")
    public List<IRoute> getRoutesStartingEndingMaxDistance(final String startTownName, final String finishTownName,
            final double maxDistance) throws UnAcceptableInputParameterException, TownNotExistException,
        ...
        final ITown startTown = getTown(startTownName);
        final ITown finishTown = getTown(finishTownName);
        final List<String> paths = getPathByDistance(maxDistance, startTown, finishTown, startTownName);
    ...
    @SuppressWarnings("null")
    private List<String> getPathByDistance(final double maxDistance, final ITown startTown, final ITown finishTown,
            final String path) throws UnAcceptableInputParameterException, TownNotExistException, RoadNotExistException {
        final List<String> returnVal = new ArrayList<String>();
        if (maxDistance < 0) {
            return Collections.emptyList();
        } else {
            for (final ITown tmpTown : startTown.getConnectedCitiesTo()) {
                final double distance = getRoad(startTown.getName(), tmpTown.getName()).getDistance();
                returnVal.addAll(getPathByDistance(maxDistance - distance, tmpTown, finishTown,
